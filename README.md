...
| 能力 | 说明 |
| --- | --- |
| ReAct Agent 循环 | `agent_loop` 反复执行「组装上下文 → 调用 LLM → 若请求工具则执行并回填结果 → 再调用」，直到 LLM 给出最终答复；对话中每步都打印工具调用与参数，过程可观测 |
| 工具系统 | 7 个基础工具：`read_file` / `write_file` / `edit_file` / `grep` / `bash` / `list_dir` / `search_web`（联网搜索），外加 `load_skill`，共 8 个；bash 带 30s 超时，grep 自动跳过 `.git`/`node_modules` 等目录 |
...
| `agent/` | 智能体层 | `agent.py` 主循环；`session.py` AgentState 会话状态与持久化；`context.py` token 用量跟踪与上下文压缩；`memory.py` 跨会话长期记忆；`skill.py` SKILL.md 发现与解析；`system_prompt.py` 行为约束（先理解再修改 / 优先获取真实信息 / 控制工具调用等中文工作准则） |
...
│   ├── context.py           # ContextManager：token 统计 / compact / auto_compact(≥80%)
...
│   └── my_tools/            # basic: bash/grep/ls/read/write/edit   usual: search_web
└── tests/test_permission.py # unittest：权限放行/询问/拒绝（mock 用户输入）
```
